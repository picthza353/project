        ...
        echo ("<script>
            $(document).ready(function() {
                Swal.fire({
                    ...
                    text: 'ชื่อผู้ใช้งาน หรือรหัสผ่าน ไม่ถูกต้องไม่สามารถเข้าสู่ระบบได้',
...
function saveRegister($firstname, $lastname, $telephone, $email, $address, $username, $password) {
    ...
    $sql = "INSERT INTO users (firstname, lastname, telephone, email, address, username, password, role) VALUES ($1, $2, $3, $4, $5, $6, $7, '2')";
    $params = array($firstname, $lastname, $telephone, $email, $address, $username, $password);
	...
	foreach( $products_id as $key => $pd ) {
		if ($pd != "") {
			...
			if($types == 1){
				...
				$str = $dateNow."\n ขายสินค้าเสร็จสิ้น \n ชื่อร้าน: ".$customer_name."\n เบอร์โทรศัพท์: ".$customer_telephone."\n ชื่อสินค้า: ".$data['prod_name']."\n น้ำหนัก: ".$pa."\n ราคารวม: ".$sm;
...
function getCurrentContact(){
	...
	$result=pg_fetch_array($res,PG_ASSOC);
	return $result;
