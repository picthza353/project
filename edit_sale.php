...
if($_GET["id"] == ""){
  $txtHead = "สร้างรายการขาย";
}else{
  $txtHead = "แก้ไขรายการขาย";
}
...
<body>
      ...
      <form action="" method="post" enctype="multipart/form-data">
        <div class="row">
          <div class="col-lg-12">
            ...
            <div class="row">
              <input type="hidden" class="form-control" id="types" name="types" value="1">
              <div class="col-md-6">
                <div class="form-group">
                  <label>เล่มที่</label>
                    ...
                    <tr>
                      ...
                      <td style="width:35%;">
                        <select name="products_id[]" id="products_id<?php echo $i;?>" class="form-control product" required>
                          <option value="" disabled selected>-- โปรดเลือก --</option>
                          <?php foreach($allStock as $dataPro1){ ?>
                      ...
                      <td style="width:20%;">
                        <input type="text" class="form-control border-input price" name="price[]" id="price<?php echo $i;?>" required>
                      </td>
                      <td style="width:20%;">
  ...
  <script language="javascript">
    ...
    function addRow(tableID) {
      ...
      <?php foreach($allStock as $dataPro2){ ?>
        var option = document.createElement("option");
      ...
      element3.addEventListener('change', calTotalPriceFromPrice);
  ...
  <script>
    function getDataFromDropdown(){
      ...
      $.get('api/api.php?load=product&pro_id='+pro_id,function(data){
        ...
        for (var j = 0, len = pro_line_new.length; j < len; j++) {
          $('#amount'+sub_id).val(pro_line_new[j].prod_amount);
        }
      });
    ...
    function calTotalPriceFromAmount(){
      ...
      var prod_price = parseInt($('#price'+sub_id).val(), 10);
      var prod_amount = parseFloat($('#amount'+sub_id).val(), 10);
      var total_price = parseInt(prod_price*prod_amount);
      var prod = $('#products_id'+sub_id).val();
      var amount = $('#amount'+sub_id).val();
      var price = $('#price'+sub_id).val();

      $.get('api/api.php?load=product&pro_id='+prod,function(data){
        pro_line_new = jQuery.parseJSON(data);
        for (var j = 0, len = pro_line_new.length; j < len; j++) {        
            if(prod_amount > pro_line_new[j].prod_amount){
              Swal.fire({
                icon: 'error',
              ...
              $('#amount'+sub_id).val(pro_line_new[j].prod_amount);
            }else if(!isNaN(amount) && amount != "" && price != ""){
    ...
    function calTotalPriceFromPrice(){

      var id = $(this).attr('id');
      var sub_id = id.substring(7, 5);
      var prod_amount = parseFloat($('#amount'+sub_id).val(), 10);
      var prod_price = parseInt($('#price'+sub_id).val(), 10);
